/* Hero tool — vertical: Short link / QR toggle, manual shorten, customizable slug, 7-day-free, real QR */
(function hero2(){
  ...
  let mode="link";
  const heroVar=(new URLSearchParams(location.search).get("hero")==="a")?"a":"b"; /* b = ranked picker (default), a = single + chips */
  let domain="i.ki";
  const DOMAINS=[
    {d:"i.ki",tag:"Free",note:"Default — live instantly"},
  ...
  function aiAlts(dest,primary){
    let host="";
    try{const x=new URL(/^https?:/.test(dest)?dest:"https://"+dest);const ps=x.hostname.replace(/^www\./,"").split(".");host=ps.length>=2?ps[ps.length-2]:ps[0];}catch{}
    const clean=s=>(s||"").replace(/[^a-z0-9-]/gi,"").toLowerCase().replace(/-+/g,"-").replace(/^-|-$/g,"").slice(0,18);
    const base=clean(primary);
    const words=base.split("-").filter(Boolean);
    const yr=(base.match(/20\d\d/)||[])[0];
    const no=words.filter(w=>!/^20\d\d$/.test(w));
  ...
  function renderInput(){
    ...
    <div class="h2in">
      ${mode==="link"?`<div class="h2dom-wrap">
    ...
    <p class="h2in__hint"><span class="o">✦</span> AI suggests a name as you paste — use your <b>custom domain</b>, and <b>free for 7 days</b>.</p>`;
    ...
    if(domBtn){
      const markSel=()=>domList.querySelectorAll(".h2domitem").forEach(b=>b.classList.toggle("is-sel",b.dataset.d===domain));
      ...
      domList.addEventListener("click",e=>{ const it=e.target.closest(".h2domitem"); if(!it)return; if(it.dataset.add){ closeDom(); toast("Custom domains are available on the Pro plan"); return; } domain=it.dataset.d; const t=card.querySelector(".h2in__domtxt"); if(t)t.textContent=domain; markSel(); closeDom(); url.focus(); });
    }
  }

  function noteHTML(kind){ return `<div class="h2note">${IC.check}<span><b>Live now</b> — ${kind} works instantly, no login. Free for <b>7 days</b>, then it expires. <a href="#">Sign in</a> to keep it forever.</span></div>`; }

  /* ---- short link result (dispatch A/B) ---- */
  function renderLink(slug,dest){ return heroVar==="a"?renderLinkA(slug,dest):renderLinkB(slug,dest); }

  /* ---- Option A: single auto-suggest + alternative chips ---- */
  ...
  function renderQR(slug,dest){
    card.innerHTML=`<div class="h2res">
      <div class="h2res__head"><span class="h2res__lbl">Your QR code</span><span class="h2res__badge">${IC.clock} Free for 7 days</span></div>
      <div class="h2res__qr">
        <div class="h2res__qrbox" id="h2qrbox">${makeQR("https://"+domain+"/"+slug)}</div>
        <div class="h2res__qrside">
          <span class="h2res__lbl2">Links to</span>
          <div class="h2res__link h2res__link--sm" id="h2linkrow"><span class="h2res__dom">${domain}/</span><input class="h2res__slug" id="h2slug" value="${slug}" spellcheck="false"/><span class="h2res__pen" title="Customize">${IC.edit}</span></div>
  ...
  /* ---- trust strip (real signals, rehomed) ---- */
  trust.innerHTML=`
    <div class="h2tp">
