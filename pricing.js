...
(function pricing(){
  ...
  const PLANS=[
    ...
    {key:"starter",pill:"Starter",price:{m:12,y:9},desc:"For solo creators and freelancers",cta:"Start free trial",type:"ghost",
     features:["Everything in Free","1 custom domain","Bulk link creation","3 workspace seats","Email support"]},
    {key:"pro",pill:"Pro",price:{m:29,y:23},desc:"For growing teams that ship everyday",cta:"Start free trial",type:"primary",popular:true,
     features:["Everything in Starter","AI Router (device, geo, A/B)","AI Insights & forecasting","10 seats & role permissions","Full API + webhooks","Priority email support"],
  ...
  const GROUPS=[
    ["Links & QR",[["Short links","Unlimited","Unlimited","Unlimited","Unlimited"],["Custom domains",false,"1","3","Unlimited"],["QR codes",true,true,true,true],["UTM builder","Basic",true,true,true],["Bulk link creation",false,true,true,true]]],
    ["Analytics",[["Click & conversion analytics",true,true,true,true],["AI Insights & forecasting",false,false,true,true],["Advanced traffic insights",false,false,false,true],["Data retention","30 days","90 days","1 year","Custom","How long event-level data stays queryable"]]],
    ["Smart routing",[["AI Router (device, geo)",false,false,true,true,"Auto-routes each visitor by device, geo and source"],["A/B testing",false,false,true,true],["Retargeting & deep links",false,false,false,true]]],
    ["Team",[["Workspace seats","1","3","10","Unlimited"],["Shared workspaces & folders",false,true,true,true],["Roles & permissions",false,false,true,true],["SSO / SAML / SCIM",false,false,false,true]]],
    ["Developer",[["API + webhooks",false,false,true,true],["White-label dashboards",false,false,false,true]]],
    ["Support & security",[["Support","Community","Email","Priority","Dedicated CSM"],["Uptime SLA",false,false,false,"99.99%"],["Audit logs",false,false,false,true],["SOC 2 + GDPR / DPA",false,false,false,true]]],
  ];
  ...
  const groups=GROUPS.map(([g,items],gi)=>{
    const body=items.map(row=>{const[n,...rest]=row;const tip=rest.length>4?rest[4]:null;const vals=rest.slice(0,4);return `<div class="prt__row"><span class="prt__feat">${n}${tip?`<span class="prt__i" title="${tip}">${info}</span>`:''}</span>${vals.map((v,i)=>`<span class="${i===2?'prt__pro':''}">${cell(v)}</span>`).join("")}</div>`;}).join("");
